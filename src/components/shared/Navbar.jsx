      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative flex justify-between items-center" ref={searchRef}>
          ...
          {navLinks.map((link) => {
            ...
            return (
              <li key={link.name} className="relative group py-2">
                <Link 
                  href={link.href || "#"} 
                  className={`relative z-10 text-[13px] font-bold uppercase transition-colors px-1 flex items-center gap-1 ${active ? 'text-pink-500' : 'text-black hover:text-pink-500'}`}
                >
                  {link.name}
        ...
        <div className="flex items-center gap-4 md:gap-6 text-black">
          <div className="relative flex items-center">
              ...
              {isSearchOpen && (
                <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: 220, opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="relative hidden lg:block mr-2">
                  <input 
                    type="text" 
                    placeholder="Search..." 
                    className="w-full bg-gray-50 border border-gray-100 rounded-full px-5 py-2 text-sm outline-none focus:ring-2 focus:ring-pink-500/10"
                    value={searchQuery}
                    onChange={(e) => {setSearchQuery(e.target.value); if(e.target.value.trim()) setIsSearching(true);}}
                    autoFocus
                  />
            ...
            <button onClick={() => { setIsSearchOpen(!isSearchOpen); setSearchQuery(""); }} className="hover:text-pink-500 pt-1">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="absolute right-0 mt-2 bg-white shadow-2xl border border-gray-100 min-w-[180px] py-2 z-[100] rounded-xl overflow-hidden">
                  {user ? (
                    <>
                      ...
                      <Link href={user.role === 'admin' ? '/admin/dashboard' : '/profile'} onClick={() => setIsUserMenuOpen(false)} className="flex items-center gap-3 px-5 py-3 text-[11px] font-black hover:bg-gray-50 uppercase tracking-tighter text-black"><UserCircle size={16} /> {user.role === 'admin' ? 'Admin Panel' : 'My Account'}</Link>
          ...
          {isSearchOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="lg:hidden absolute top-full left-0 w-full bg-white border-b border-gray-100 shadow-lg overflow-hidden">
              <div className="px-4 py-4">
                <input type="text" placeholder="Search..." className="w-full bg-gray-50 border border-gray-200 rounded-xl px-5 py-3 text-sm outline-none" value={searchQuery} onChange={(e) => {setSearchQuery(e.target.value); if(e.target.value.trim()) setIsSearching(true);}} autoFocus />
                {searchQuery && (
                  <div className="mt-3 max-h-64 overflow-y-auto space-y-2">
                    {filteredResults.map(p => (
                      <div key={p._id} onClick={() => handleProductClick(p._id)} className="flex items-center gap-4 p-2 active:bg-gray-100 rounded-lg">
                        ...
                        <div className="flex flex-col"><span className="text-xs font-bold text-gray-800">{p.name}</span><span className="text-[10px] text-pink-500 font-black">${p.salePrice}</span></div>
                      </div>
                    ))}
                  </div>
                )}
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: 'spring', damping: 25 }} className="fixed top-0 left-0 w-[280px] h-full bg-white z-[70] p-6 shadow-2xl lg:hidden">
              <div className="flex justify-between items-center mb-8 border-b border-gray-50 pb-6"><Image src={img} alt="Logo" className="w-[100px]" /><button onClick={() => setIsMobileMenuOpen(false)} className="p-2 bg-gray-50 rounded-full"><X size={20} /></button></div>
                ...
                {navLinks.map((link) => (
                  <li key={link.name} className="border-b border-gray-50 last:border-0">
                    <div className="flex justify-between items-center py-4" onClick={() => link.hasDropdown && setOpenMobileDropdown(openMobileDropdown === link.name ? null : link.name)}>
                      <Link href={link.href || "#"} onClick={!link.hasDropdown ? closeAll : undefined} className={`text-[13px] font-black uppercase tracking-tight transition-colors ${isActive(link.href) ? 'text-pink-500 border-l-2 border-pink-500 pl-2' : 'text-black'}`}>{link.name}</Link>
                      {link.hasDropdown && <ChevronDown size={18} className={`${openMobileDropdown === link.name ? 'rotate-180' : ''} transition-transform`} />}
                    </div>
                    {link.hasDropdown && openMobileDropdown === link.name && (
                      <motion.ul initial={{ height: 0 }} animate={{ height: 'auto' }} className="pl-4 pb-2 overflow-hidden bg-pink-50/30 rounded-lg">
                        {link.items.map(item => (<li key={item}><Link href={`/shop?category=${item.toLowerCase()}`} onClick={closeAll} className="py-3 px-4 text-[11px] font-bold text-gray-600 block uppercase">{item}</Link></li>))}
                      </motion.ul>
                    )}
                  </li>
                ))}
